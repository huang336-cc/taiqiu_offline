...
// v1.3.58：放宽最大放大倍率。
// 旧值 ZOOM_MIN=0.35 / SAFE_CLEAR=0.7 在横屏下算出的缩放下限是 0.756，
// 也就是说球杆最多只能放大到「刚溢出屏幕一点点」，想看杆头雕花、缠线纹理
// 根本不够用。现在把绝对下限放到 0.25、安全间隙收到 0.2（仍为 near=0.1 的
// 两倍），横屏下限降到约 0.633（放大能力提升约 19%），竖屏降到约 0.273
// （提升约 22%）。0.2 是保守值：即便相机转到杆的轴线延长线上（最坏情况），
// 杆尾到相机仍有 0.2 的间隙，不会被 near 平面切掉。
var ZOOM_MIN = 0.25 // 缩放绝对下限（兜底，见下方 SAFE_CLEAR 说明）
var ZOOM_MAX = 1.6
...
// v1.3.58：用户反馈放大倍率不够用，SAFE_CLEAR 由 0.7 收到 0.2 ——
// 横屏下限降到约 0.633、竖屏约 0.273，放大能力分别提升约 19% / 22%。
// 0.2 仍给足两倍 near(0.1) 的余量：即便相机转到球杆轴线的延长线上（最坏角度），
// 杆尾到相机也还有 0.2 的间隙，不会被近平面切穿。
var SAFE_CLEAR = 0.2
