 ...
 * 贴图种类。
 *
 *  - `carpet`   —— 方块地毯（office 主地面）
 *  - `wood`     —— 橡木地板（room，细密直纹）
 *  - `stone`    —— 深色地砖（cybercafe，哑光带细微颗粒）
 ...
 * 深色地砖：哑光微颗粒。
 *
 * cybercafe 的地砖本身颜色很深（顶点色负责），贴图只补**细微颗粒感**，
 * 让掠射下的大片地面不至于是一块死板渐变。
 *
 * ⚠️ 明度钳在 [240, 255]（线性 ≥0.871），见文件头铁律。
 */
 ...
 * 水泥/环氧：大尺度斑驳 + 细骨料。
 *
 * 用在「走道」这类次级地面（office 的过道、cyber 的过道）。它要在
 * 视觉上比主地面**更粗糙** —— 低频斑驳最明显，这是三张里对比最大的一张。
 *
 * ⚠️ 明度钳在 [240, 255]（线性 ≥0.871），见文件头铁律。
 */
